...
import { cancelProject, deliverWork, markAsDelivered } from "../../redux/slices/projectSlice";
...
function ProjectCard({ project }) {
  ...
  const isActionDisabled = (action) => {
    ...
    if (action === "markAsDelivered") {
      return project.status === "completed-reviewed";
    }
    if (action === "cancel") {
      return project.status === "completed-reviewed";
    }
      ...
      <div className="flex items-center mt-3 justify-between flex-wrap gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center text-gray-500 text-sm">
            ...
            {deadline ? (
              <span className={isLate ? "text-red-500" : "text-gray-500"}>
                {deadline.format("MMM D, YYYY")} {isLate && "(Late)"}
              </span>
            ) : (
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={isActionDisabled(
              user?.role === "client" ? "markAsDelivered" : "deliver"
            )}
          ...
          >
            ...
            {user?.role === "client" ? "Mark as Complete" : "Deliver Work"}
          </button>
          ...
          <button
            ...
            className={`flex items-center gap-1 border border-primary text-primary px-3 py-1 rounded-md text-sm hover:bg-primary hover:text-white transition cursor-pointer ${
              isActionDisabled("cancel") ? "opacity-50 pointer-events-none" : ""
            }`}
          >
